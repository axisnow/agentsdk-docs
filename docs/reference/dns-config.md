...
!!! note "白名单 / 豁免会自动归一化"
    两份列表在初始化时会做归一化（首尾空白裁剪、小写、去掉末尾 `.`、IDN 域名转 ASCII/Punycode；`*.suffix` 仅归一化后缀部分，`*` 保持原样；归一化后为空的条目被丢弃），与待解析域名的归一化口径一致。因此 `API.Example.COM.`、`api.example.com`、`*.Bücher.example` 这类写法都会被正确匹配，无需手动统一大小写或转码。
