...
所有 SDK API 返回 `0` 表示成功，负数表示失败。错误码按子系统分段（通用 / 初始化&配置 / DNS / Proxy），完整定义见 [错误码总表](errors.md)。
...
2. **检查 API 返回值**：`getLocalTCPProxy` 等是否返回有效结果？
3. **检查网络连接**：设备是否能访问 Edge 节点？
4. **检查 DNS 配置**：`edgedoh_resolve_domains` / `edgedoh_bypass_domains` 配置是否符合预期？
...
#### AccessKey / EdgeAddresses 类（`-112` / `-113` / `-114` / `-115`）

**现象**：`initialize` 返回 `-112` ~ `-115`

**排查**：
- `-112` `AccessKeyIDMissing`：配置里 `access_key_id` 没填或填了空字符串
- `-113` `AccessKeySecretMissing`：`access_key_secret` 没填或填了空字符串
- `-114` `AccessKeyInvalid`：值填了但解析失败 —— 可能复制时漏了字符、加了空白、用错了凭证；从控制台重新复制
- `-115` `EdgeAddressEmpty`：`edge_addresses` 是空数组或全是空字符串；至少配置一个有效地址
...
- 字段类型与 [接入指南附录 A](guides/index.md)定义不符
...
**现象**：`getLocalTCPProxy` 返回负数错误码

**排查**：
- `-101` `NotInitialized`：SDK 未初始化，检查初始化时序
- `-2` `InvalidArgument`：参数非法
  - `host` 是否为空或包含非法字符？
  - `port` 是否在 1-65535 范围内？
  - `host` 是否是合法的域名或 IP 地址？
...
**排查**：
- 确认域名拼写正确
- 检查域名是否命中 `dns.edgedoh_resolve_domains`（精确 / `*.suffix` 通配）
- 检查域名是否被 `dns.edgedoh_bypass_domains` 意外豁免（bypass 优先级高于 resolve）
- 如果走 EdgeDoH 路径，检查 Edge 节点是否可达
- 确认 `dns.fallback: true`（默认开启），允许 hedged 兜底到系统 DNS
...
- **业务域名启用 EdgeDoH**：把需要防劫持的域名加入 `dns.edgedoh_resolve_domains`（注意：默认不启用，需显式配置）
- **特殊域名豁免**：登录域名等需保持系统 DNS 行为的，加入 `dns.edgedoh_bypass_domains`（bypass 优先于 resolve）
